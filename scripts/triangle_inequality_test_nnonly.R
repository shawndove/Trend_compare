ti.test.nn.fn1 <- function(dist.fun, 
                        print_name, 
                        dist.args,
                        tpops=100000,
                        tmax=5) 
{
  ...
  source("scripts/generate_time_series_nn.R")
  
  # store sequence of years for plotting purposes
  #years <- seq(1,100)
  
  # create 3 lists of random time series using pgrowth_nn function
  # it is important to get the settings right, so that some 0s will be created (negative
  # values are automatically adjusted to 0, and that some values in list 2 will be higher than in list 1,
