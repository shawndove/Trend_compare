                          ...
                          y)
{
  ...
  temp_nn <- readRDS(file = paste(wd, "/files/nn_results/", print_name, "_nnfull.RData", sep=""))
  ...
  temp_ti <- readRDS(file = paste(wd, "/files/ti_results/", print_name, "_tifull.RData", sep=""))
  
  test.results$"Triangle Inequality" <- temp_ti[[length(temp_ti)]]
  ...
  if (!is.list(dist.args)) 
  {
    ...
    for (i in seq_along(tslistx.uni)) 
    {
      ...
      # use this line to call the distance measure fn if there are no extra arguments
      try(test.results$"Uniform Time Scaling Invariance"[[i]] <- dist.fun(tslistx.uni[[i]], 
                                                                      tslisty.uni[[i]]))
  ...
  } else 
  {
    ...
    for (i in seq_along(tslistx.uni)) 
    {
      ...
      # use this line if there are extra arguments
      try(test.results$"Uniform Time Scaling Invariance"[[i]] <- do.call(dist.fun, 
                                                                                  argslist.uni))
  ...
  if(length(test.results$"Uniform Time Scaling Invariance")==0)
    {
    
    test.results$"Uniform Time Scaling Invariance" <- c(NA, NA, NA, NA)
  ...
  if (!is.list(dist.args)) 
  {
    ...
    for (i in seq_along(tslistx.warp)) 
    {
      ...
      # use this line to call the distance measure fn if there are no extra arguments
      try(test.results$"Warping Invariance"[[i]] <- dist.fun(tslistx.warp[[i]], 
                                                         tslisty.warp[[i]]))
    }
  ...
  } else 
  {
    ...
    for (i in seq_along(tslistx.warp)) 
    {
      ...
      # use this line if there are extra arguments
      try(test.results$"Warping Invariance"[[i]] <- do.call(dist.fun, 
                                                                     argslist.warp))
  ...
  if(length(test.results$"Warping Invariance")==0)
  {
    
    test.results$"Warping Invariance" <- c(NA, NA, NA, NA)
  ...
  if (is.na(max(test.results$"Warping Invariance"))) 
  {
  ...
  } else if (max(abs(test.results$"Warping Invariance")) < 0.01) 
  {
  ...
  } else 
  {
    ...
    ymin.warp <- min(test.results$"Warping Invariance") - 0.1*min(abs(test.results$"Warping Invariance"))
    
    ymax.warp <- max(test.results$"Warping Invariance") + 0.1*max(abs(test.results$"Warping Invariance"))
  ...
  # testing: Non-positive Value Handling ----
  ...
  if (!is.list(dist.args)) 
  {
    ...
    # loop to test each distance measure for Non-positive Value Handling
    for (i in seq_along(tslistx.np)) 
    {
      
      # use this line to call the distance measure fn if there are no extra arguments
      test.results$"Non-positive Value Handling"[[i]] <- dist.fun(tslistx.np[[i]], 
                                                            tslisty.np[[i]])
  ...
  } else 
  {
    ...
    for (i in seq_along(tslistx.np)) 
    {
      ...
      argslist.np[[1]] <- tslistx.np[[i]]
      
      argslist.np[[2]] <- tslisty.np[[i]]
      
      names(argslist.np)[1] <- x
      
      names(argslist.np)[2] <- y
      
      for (j in 1:length(dist.args)) 
      {
        
        argslist.np[2+j] <- dist.args[j]
        
        names(argslist.np)[2+j] <- names(dist.args[j])
        
      }
      
      # use this line if there are extra arguments
      test.results$"Non-positive Value Handling"[[i]] <- do.call(dist.fun, argslist.np)
      
    }
    
  }
  
  plot.res$"Non-positive Value Handling" <- data.frame(seq_along(tslistx.np),
                                                 test.results$"Non-positive Value Handling")
  
  colnames(plot.res$"Non-positive Value Handling") <- c("x", "y")
  
  if (is.na(max(test.results$"Non-positive Value Handling"))) 
  {
  ...
  } else if (max(abs(test.results$"Non-positive Value Handling")) < 0.01) 
  {
  ...
  } else 
  {
    ...
    ymin.np <- min(test.results$"Non-positive Value Handling") - 0.1*min(abs(test.results$"Non-positive Value Handling"))
    
    ymax.np <- max(test.results$"Non-positive Value Handling") + 0.1*max(abs(test.results$"Non-positive Value Handling"))
  ...
  if (!is.na(ymin.uni)) 
  {
    ...
    p8 <- ggplot(plot.res$"Uniform Time Scaling Invariance",
                  aes(x = x, 
                      y = y)) +
      geom_point(size=2) +
      ylim(ymin.uni, ymax.uni) +
      theme(legend.title = element_text(color="blue")) +
      ggtitle("Uniform Time Scaling Invariance") +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5),
            axis.title.x = element_blank(),
            axis.title.y = element_blank(),
  ...
  if (!is.na(ymin.warp)) 
  {
    ...
    # save ggplot of Antiparallelism Bias
    p9 <- ggplot(plot.res$"Warping Invariance",
                  aes(x = x, 
                      y = y)) +
      geom_point(size=2) +
      ylim(ymin.warp, ymax.warp) +
      theme(legend.title = element_text(color="blue")) +
      ggtitle("Warping Invariance") +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5),
            axis.title.x = element_blank(),
            axis.title.y = element_blank(),
  ...
  ggsave(paste(wd, "/plots/test_results_4/", print_name, "_plots.jpg", sep=""), p.all)
  
  # write the flextable as a Word document
  save_as_docx(tr_table, 
               path = paste(wd, "/tables/test_results_4/", print_name, "_table.docx", sep=""))
  
  # write the wide-form data table to an R file
  saveRDS(tr_wide, file = paste(wd, "/files/wide4/", print_name, "_dfwide.RData", sep=""))
